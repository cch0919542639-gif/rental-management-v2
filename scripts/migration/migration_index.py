...
SCRIPT_NOTES = {
    ...
    "prepare_target_db.py": {
        "mode": "prepare",
        "type": "target scaffold",
        "safety": "review-required",
        "requires_review": "yes",
        "verification": "Dry-run first, then confirm row counts remain zero after execute on a rehearsal target.",
        "rollback": "Recreate the target database from scratch or restore it from backup.",
        "description": "Inspect or prepare the rehearsal target schema before CSV import.",
    },
    "write_rehearsal_evidence.py": {
        "mode": "evidence",
        "type": "audit bundle",
        "safety": "safe",
        "requires_review": "no",
        "verification": "Preview JSON first, then confirm the written bundle paths and summary fields.",
        "rollback": "Delete the generated evidence JSON file if needed.",
        "description": "Summarize manifest, parity, and checklist artifacts into a standard evidence bundle.",
    },
    "migration_index.py": {
        "mode": "index",
        "type": "index",
