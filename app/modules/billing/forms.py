...
class MonthlyBillForm(FlaskForm):
    ...
    rent = DecimalField("租金", validators=[DataRequired(), NumberRange(min=0)], places=0)
    ...
    electricity_amount = DecimalField("電費", validators=[Optional(), NumberRange(min=0)], places=0)
    public_electricity = DecimalField("公電", validators=[Optional(), NumberRange(min=0)], places=0)
    ...
    water_amount = DecimalField("水費", validators=[Optional(), NumberRange(min=0)], places=0)
    other_charges = DecimalField("其他費用", validators=[Optional(), NumberRange(min=0)], places=0)
    other_desc = StringField("其他費用說明", validators=[Optional()])
    previous_balance = DecimalField("前期未收餘額", validators=[Optional(), NumberRange(min=0)], places=0)
