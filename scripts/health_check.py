...
def main() -> int:
    ...
    from app.core.config.validation import _detect_db_dialect
    ...
    if config.get("SQLALCHEMY_DATABASE_URI", "").startswith("sqlite:///:memory:"):
        ...
        dialect = _detect_db_dialect(config["SQLALCHEMY_DATABASE_URI"])
        ok(f"Database ({dialect}): {config['SQLALCHEMY_DATABASE_URI'][:60]}...")

    for issue in get_runtime_config_issues(app):
        if issue["severity"] == "warning":
            print(f"  [WARN] {issue['code']}: {issue['message']}")
        elif issue["severity"] == "error":
            fail(f"{issue['code']}: {issue['message']}")
            errors += 1
